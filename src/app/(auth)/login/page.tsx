...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.error) {
        if ('code' in result && result.code === 'account_locked') {
  ...
  return (
    <>
      ...
      {/* Form header */}
      <div className="mb-8">
        <h1 className="font-display text-3xl sm:text-4xl font-light text-noir-50 tracking-wide mb-2">Sign In</h1>
        <p className="font-body text-noir-300 text-sm">Welcome back</p>
      </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 py-3 rounded-xl bg-noir-700/60 text-noir-100 border border-noir-500/30 hover:border-gold-400/30 font-body text-sm transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed mb-6"
      >
        {googleLoading ? (
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex-1 h-px bg-noir-600/50" />
        <span className="font-body text-xs text-noir-500 uppercase">or</span>
        <div className="flex-1 h-px bg-noir-600/50" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex justify-end -mt-1">
          <Link href="/forgot-password" className="font-body text-xs text-noir-400 hover:text-gold-400 transition-colors">
            Forgot password?
          </Link>
        </div>
      ...
      <p className="text-center font-body text-sm text-noir-300 mt-8">
        ...
        <Link href="/register" className="text-gold-400 hover:text-gold-300 transition-colors">Sign up</Link>
      </p>

      {/* Terms notice for Google sign-in */}
      <p className="text-center font-body text-xs text-noir-500 mt-4">
        By signing in, you agree to our{' '}
        <Link href="/terms" className="underline hover:text-noir-300">Terms</Link> and{' '}
        <Link href="/privacy" className="underline hover:text-noir-300">Privacy Policy</Link>.
      </p>
    </>
  )
}
